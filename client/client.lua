            ...
            for i = 1, #creeatedate, 1 do
                if tonumber(bankdata[i].emprunt) == 0 then 
                    RageUI.Button(Trad["loan_menu_title"], Trad["loan_menu_descrition"], {}, true, {
                        onSelected = function()
                            amount = KeyboardInput("Default", Trad["input_loan"], '', 10)
    ...
    while true do
        ...
        for k,v in pairs(Position.ATM) do
            if #(PlyCoord - v.poss) < 1.5 then
                ...
                --ESX.ShowFloatingHelpNotification('Appuyez sur ~g~E~s~ pour accèder a ~b~l ATM', vector3(v.posi.x, v.posi.y, v.posi.z))
