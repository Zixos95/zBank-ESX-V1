...
AddEventHandler('zBank:createaccount', function(name, firstname, age, sex, nationality)
    ...
    if xPlayer.getMoney() >= pricecard then 
        xPlayer.removeMoney(pricecard)

        MySQL.Async.fetchAll("SELECT * FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result)
            if result[1] then 
                TriggerClientEvent('esx:showNotification', _source, Trad["already_have_account_1"]..pricecard..Trad["symbol_money"]..Trad["already_have_account_2"])
            else
                ...
                TriggerClientEvent('esx:showNotification', _source, Trad["createaccount_succes"])
        ...
        TriggerClientEvent('esx:showNotification', _source, Trad["enough_money"])
    end
end)
...
AddEventHandler("zBank:getcash", function()
    ...
    local moneyped = xPlayer.getMoney()
...
AddEventHandler('zBank:gethaveaccount', function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result)
        if result[1] == 0 then 
            TriggerClientEvent('esx:showNotification', _source, Trad["dont_have_account"])
        elseif items == 0 then 
            TriggerClientEvent('esx:showNotification', _source, "Vous n'avez pas de carte bancaire")
...
AddEventHandler('zBank:verifcreate', function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result)
        if result[1] then 
            TriggerClientEvent('esx:showNotification', _source, Trad["have_account"])
        else 
...
AddEventHandler('zBank:depositmoney', function(amount)
    ...
    if xPlayer.getMoney() >= count then 
        ...
        TriggerClientEvent('esx:showNotification', _source, Trad["srv_deposit_1"]..count..Trad["symbol_money"]..Trad["onaccount"])
...
AddEventHandler('zBank:withdrawmoney', function(amount)
    ...
    if xPlayer.getAccount(get_bank).money >= count then 
        ...
        TriggerClientEvent('esx:showNotification', _source, Trad["srv_withdraw_1"]..count..Trad["symbol_money"]..Trad["de"]..Trad["onaccount"])
    ...
    if maxemprunt >= count then 
        ...
        MySQL.Async.fetchAll("SELECT emprunt FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result)
            if tonumber(result[1].emprunt) == 0 then 
                ...
                xPlayer.addAccountMoney(get_bank, count)
                TriggerClientEvent('esx:showNotification', _source, Trad["success_loan"]..count..Trad["symbol_money"]..Trad["success_loan2"])
                TriggerClientEvent('esx:showNotification', _source, Trad["loan_pay_max"])
                ok = false
            else
                TriggerClientEvent('esx:showNotification', _source, Trad["have_loan"])
            end
        end)
    else 
                ...
                if tonumber(result) ~= 157 then
                    if tonumber(result) <= os.time() then
                        MySQL.Async.fetchScalar('SELECT montantemprunt FROM bank_account WHERE identifier = @identifier', {
                            ...
                            TriggerClientEvent('esx:showNotification', _source, Trad["loan_timeout"]..dontbuy..Trad["symbol_money"])
                        end)
                        MySQL.Async.execute("UPDATE bank_account SET `emprunt` = @emprunt, `montantemprunt` = @montantemprunt, `expirationemprunt` = @expirationemprunt, `dateemprunt` = @dateemprunt WHERE identifier = @identifier", {
                            ['@identifier'] = xPlayer.identifier,
                            ['@emprunt'] = 0,
                        ...
                        TriggerClientEvent('esx:showNotification', _source, Trad["loan_print_time1"]..math.floor(day)..Trad["loan_print_time2"]..txthrs..Trad["loan_print_time3"])
                    end
                end
        ...
        if xPlayer.getAccount(get_bank).money >= count then 
            ...
            MySQL.Async.fetchAll("SELECT * FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result)
                if tonumber(result[1].emprunt) == 1 then 
                            ...
                            MySQL.Async.execute("UPDATE bank_account SET `montantemprunt` = @montantemprunt WHERE identifier = @identifier", {
                                ...
                                MySQL.Async.fetchAll("SELECT montantemprunt FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result3)
                                    if tonumber(result3[1].montantemprunt) == 0 then 
                                        ...
                                        TriggerClientEvent('esx:showNotification', _source, Trad["success_loan_pay"])
                                    else
                                        TriggerClientEvent('esx:showNotification', _source, Trad["failed_loan_pay"])
                                    end
                                end)
                            end)
                        else        
                            MySQL.Async.fetchAll("SELECT montantemprunt FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result4)
                                ...
                                MySQL.Async.execute("UPDATE bank_account SET `montantemprunt` = @montantemprunt WHERE identifier = @identifier", {
                                    ...
                                    MySQL.Async.fetchAll("SELECT montantemprunt FROM bank_account WHERE identifier = @identifier", {['identifier'] = xPlayer.identifier}, function(result5)
                                        ...
                                        TriggerClientEvent('esx:showNotification', _source, Trad["mid_pay_loan"]..tonumber(newmoney)..Trad["symbol_money"])
                                        TriggerClientEvent('esx:showNotification', _source, Trad["success_loan_pay"])
                                    end)
                                end)
                            end)
                    ...
                    TriggerClientEvent('esx:showNotification', _source, Trad["no_have_loan"])
                end
            end)
        else 
            TriggerClientEvent('esx:showNotification', _source, Trad["enough_money"])
    ...
    if xPlayer.getAccount(get_bank).money >= count then
        ...
        MySQL.Async.fetchAll('SELECT * FROM bank_account WHERE rib = @rib', {
            ...
            if result[1] ~= nil then 
                ...
                xPlayer.removeAccountMoney(get_bank, count)
                TriggerClientEvent('esx:showNotification', _source, Trad["send_payment"]..count..Trad["symbol_money"]..Trad["send_payment2"]..ribtarget)
                --addHistorique(Trad["historique_payment"], count)
            else 
                TriggerClientEvent('esx:showNotification', _source, Trad["rib_not_exist"])
            end 
        end)
...
AddEventHandler('zBank:getvirement', function(virementid)
    ...
    MySQL.Async.fetchAll('SELECT * FROM bank_virement WHERE id = @id', {
        ...
        if result[1].auth == Trad["recuperable"] then 
            ...
            TriggerClientEvent('esx:showNotification', _source, Trad["get_payment"]..result[1].id)
        else 
            TriggerClientEvent('esx:showNotification', _source, Trad["already_get_payment"])
        end
...
AddEventHandler("zBank:buycarte", function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    if xPlayer.getMoney() >= tonumber(pricecard) then 
        xPlayer.removeMoney(pricecard)
        xPlayer.addInventoryItem(carditem, 1)
        TriggerClientEvent('esx:showNotification', _source, Trad["buy_card"])
    else 
        TriggerClientEvent('esx:showNotification', _source, Trad["enough_money"])
    end
